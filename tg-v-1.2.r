...
balls_fil %>%
  ggplot(aes(x = date, y = total_balls_estimation_mean, fill = day, label = total_balls_estimation_mean)) +
  geom_col() +
  geom_text(vjust = 2, size = 4) +
  labs(x = "", y = "Estimated Balls Dispensed") +
...
balls_fil %>%
  ...
  geom_col() +
  geom_text(vjust = 2) +
  labs(x = "", y = "Practice Balls Dispensed") +
  theme(axis.text.x = element_text(angle = 90, vjust = .5), legend.title = element_blank())